            ...
            if line.startswith('.'):
                ...
                if directive == '.macro':
                    if self.in_macro_definition:
                        raise SyntaxError(
                            f"Nested macro definition not allowed: {line}")
                    ...
                    self.current_macro = MacroDefinition(
                        macro_name, macro_params, [])
                    self.macro_body = []
                    continue
                    ...
                    if self.current_macro is None:
                        ...
                        self.macro_body = []
                        raise SyntaxError(
                            f"Internal error: .endmacro encountered but no macro is open: {line}")
                    ...
                    if len(directive_parts) < 2:
                        raise SyntaxError(
                            f"Invalid conditional directive: {line}")
                        ...
                        if parent_active:
                            try:
                                for symbol, value in self.symbols.items():
                                    condition = re.sub(
                                        r'\b' + re.escape(symbol) + r'\b', str(value), condition)
                    ...
                    self.conditional_stack.append(ConditionalBlock(
                        directive, condition, active and parent_active))
                    continue
                    ...
                    if not current_block.has_else:
                        ...
                        raise SyntaxError(
                            f"Multiple .else directives in the same conditional block: {line}")
                ...
                if macro_name in self.macros:
                    ...
                    expanded_processed = self.process_lines(
                        expanded_lines, base_dir)
